...
# Set these env vars to get a logged-in sessionid cookie (strongly recommended)
...
#  Converts Playwright cookie dicts → Netscape format file directly.
#  This is the KEY FIX: we no longer rely on yt-dlp --cookies-from-browser
#  for Instagram because yt-dlp rejects https://www.instagram.com/ as a
#  downloadable URL and the extraction fails with "Unsupported URL".
# ══════════════════════════════════════════════════════════════════════════════
def _write_netscape_cookies(cookies: list, filepath: str) -> bool:
    """
    Write a list of Playwright cookie dicts to a Netscape-format .txt file.

    Playwright cookie dict keys used:
    ...
    try:
        ...
        for c in cookies:
            ...
            if not domain:
                continue
            # Netscape format requires domain to start with a dot for subdomain matching
            if not domain.startswith("."):
                domain = "." + domain
            include_subdomains = "TRUE" if domain.startswith(".") else "FALSE"
            path               = c.get("path", "/")
            secure             = "TRUE" if c.get("secure", False) else "FALSE"
            # Playwright gives expires as a float (Unix timestamp); -1 means session cookie
        ...
        logger.info(
            f"✅ Wrote {written} cookies to Netscape file: {filepath}"
        )
        return written > 0
...
async def generate_cookies_via_playwright(
    ...
    """
    Launch a persistent headless Chromium session, visit Instagram
    (optionally log in), then export all cookies directly to a
    Netscape-format file via _write_netscape_cookies().

    No yt-dlp subprocess is used — this avoids the "Unsupported URL" error.
    """
    logger.info(
    ...
    try:
        async with async_playwright() as p:
            ...
            # ── KEY FIX: Export cookies directly from Playwright context ──────
            # We do NOT call yt-dlp --cookies-from-browser here because
            # yt-dlp rejects https://www.instagram.com/ ("Unsupported URL").
            # Instead we grab cookies straight from the live browser context.
            ...
            ig_cookies = [
                ...
                or "facebook.com" in c.get("domain", "")  # IG shares FB cookies
            ]

            if not ig_cookies:
                logger.error("❌ No Instagram cookies found in browser context")
                return False

            logger.info(
                f"🍪 Collected {len(ig_cookies)} Instagram cookies from browser"
            )
            ok = _write_netscape_cookies(ig_cookies, COOKIE_FILE)
            if ok:
                logger.info(
                    "✅ Instagram browser profile cookies written successfully"
                )
            return ok
...
async def refresh_cookies_from_browser(
    ...
    """
    Acquire the global lock, run Playwright, write the Netscape cookie file,
    verify it, then return COOKIE_FILE on success or None on failure.
    """
...
def verify_cookies_file(filename: str) -> bool:
    """
    Validate that the cookie file is Netscape-format and contains
    at least the minimum set of Instagram cookies.
    """
    try:
        ...
        # Cookies present even for unauthenticated / guest sessions
        guest_cookies = ["mid", "ig_did", "csrftoken", "datr"]
        # Cookies only present after login
        ...
        if len(found_all) < 2:
            logger.warning(
                f"⚠️ Too few Instagram cookies found: {found_all}"
            )
            return False
...
def is_cookie_file_expired(filepath: str) -> bool:
    ...
    if min_exp is None:
        # Session-only cookies (expiry=0) are fine; treat as unexpired
...
async def get_cookies(force_refresh: bool = False) -> Optional[str]:
    ...
    Auto-generates / refreshes when missing, invalid, or expired.
    """
...
async def download_with_ytdlp(
    ...
    Auto-refreshes cookies on auth/rate-limit errors (one retry).
    """
    ...
    for attempt in range(2):
        try:
            ...
            if is_auth_error(e) and AUTO_REFRESH_COOKIES and attempt == 0:
                ...
                if new_cookie:
                    logger.info(
                        "✅ Fresh Instagram cookies – retrying download ..."
                    )
...
async def startup_services():
    """
    Called once at bot startup. Ensures valid Instagram cookies exist
    before any download request arrives. Mirrors Youtube.startup_services().
    """
...
class InstagramAPI:
    ...
    async def info(self, url: str) -> Optional[dict]:
        ...
        Always returns a dict (never None) — falls back to safe defaults
        so callers like details["thumb"] never crash.
        """
        ...
        return {
            ...
            "thumbnail":    thumbnail,
            "thumb":        thumbnail,        # alias used by some callers
    ...
    async def download(self, url: str) -> tuple:
        """
        Download an Instagram reel / post video / IGTV clip.
        Returns (track_details dict, filepath str) or (False, None).
        """
    ...
    async def thumbnail(self, url: str) -> str:
        """Return the thumbnail URL (empty string if unavailable)."""
        meta = await self.info(url)
        return meta["thumb"]
    ...
    async def track(self, url: str) -> tuple:
        """
        Return (track_details dict, id str) without downloading.
        Matches the interface of YouTubeAPI.track(), SpotifyAPI.track(), etc.
        """
        meta = await self.info(url)
        track_details = {
            "title":        meta["title"],
