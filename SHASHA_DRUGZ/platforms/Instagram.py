...
# sec-ch-ua strings matched to each UA above
_SEC_CH_UA = {
...
(() => {
  ...
  // 7. Canvas noise (subtle – not detectable as override)
  const _toBlob = HTMLCanvasElement.prototype.toBlob;
  const _toDataURL = HTMLCanvasElement.prototype.toDataURL;
  const _noise = () => (Math.random() * 0.02 - 0.01);
  HTMLCanvasElement.prototype.toDataURL = function(type, quality) {
    const ctx = this.getContext('2d');
    if (ctx) {
      const img = ctx.getImageData(0, 0, this.width, this.height);
      for (let i = 0; i < img.data.length; i += 4) {
        img.data[i]   = Math.min(255, img.data[i]   + _noise() * 2);
        img.data[i+1] = Math.min(255, img.data[i+1] + _noise() * 2);
        img.data[i+2] = Math.min(255, img.data[i+2] + _noise() * 2);
      }
  ...
  // 9. Hide automation via toString tricks
...
async def _verify_login(context, page) -> bool:
    try:
        ...
        if found:
            ...
            return True
        # Fallback: navigated away from login without sessionid yet (rare race)
...
# ══════════════════════════════════════════════════════════════════════════════
#  FIX: WAIT-FOR-ERROR helper
#  Instagram shows an inline error div when credentials are wrong instead of
#  redirecting.  We detect it early so we don't waste 15 s waiting for a
#  URL change that will never come.
# ══════════════════════════════════════════════════════════════════════════════
async def _wait_for_login_result(page, timeout_ms: int = 18_000) -> str:
    ...
    while time.monotonic() < deadline:
        ...
        # ── Inline error messages (wrong password / suspicious login) ─────────
        try:
            body = (await page.inner_text("body")).lower()
            wrong_pw_signals = [
...
# ══════════════════════════════════════════════════════════════════════════════
#  SHARED FIELD-FILL + SUBMIT  (used by both classic paths)
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
#  CLASSIC LOGIN  (navigates to /accounts/login/ itself)
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
#  CLASSIC LOGIN ON CURRENT PAGE  (already navigated via click)
# ══════════════════════════════════════════════════════════════════════════════
        ...
        async with async_playwright() as p:
            context = await p.chromium.launch_persistent_context(
                ...
                args=[
                    ...
                    # FIX: exclude-switches removes the "enable-automation" infobar flag
...
async def refresh_cookies_from_browser(reason: str = "On-demand refresh") -> Optional[str]:
    async with _COOKIE_LOCK:
        ...
        if is_cookie_file_expired(COOKIE_FILE):
            logger.error("Cookie expired immediately after generation")
            return None
