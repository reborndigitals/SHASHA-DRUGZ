...
async def skip(cli, message: Message, _, chat_id):
    ...
    if "live_" in queued:
        ...
        try:
            ...
            image = None
        try:
            await SHASHA.skip_stream(chat_id, link, video=status, image=image)
        except:
        ...
        try:
            ...
            image = None
        try:
            await SHASHA.skip_stream(chat_id, file_path, video=status, image=image)
        except:
            return await mystic.edit_text(_["call_6"])
        ...
        if videoid == "telegram":
            ...
            try:
                ...
                image = None
        try:
            await SHASHA.skip_stream(chat_id, queued, video=status, image=image)
        except:
            return await message.reply_text(_["call_6"])
        if videoid == "telegram":
            button = stream_markup2(_, chat_id)
...
@app.on_callback_query(filters.regex("SHASHAmousAdmin") & ~BANNED_USERS)
...
@app.on_callback_query(filters.regex("SHASHAPlaylists") & ~BANNED_USERS)
@languageCB
    ...
    # ── end instagram block ────────────────────────────────────────────────
        ...
        if await is_active_chat(chat_id):
            ...
            run = await app.send_photo(
                ...
                photo=config.SOUNCLOUD_IMG_URL,
                caption=_["stream_1"].format(
                    config.SUPPORT_CHAT, title[:11], duration_min, user_name
                ),
                reply_markup=InlineKeyboardMarkup(button),
            )
            db[chat_id][0]["mystic"] = run
