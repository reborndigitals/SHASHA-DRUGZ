...
non_command = filters.create(lambda _, __, m: not (m.text and m.text.startswith("/")))
...
async def _is_approval_on(chat_id: int) -> bool:
    ...
    # Strict True check — prevents truthy DB value bugs
...
async def approve_user(client, message: Message):
    ...
    await message.reply_text(
        ...
        f"They can now send messages freely and bypass flood/locks."
    )
...
# group=3 — runs BEFORE flood checker (group=6)
@app.on_message(filters.group & ~filters.service & non_command & ~BANNED_USERS, group=3)
async def enforce_approval(client, message: Message):
    ...
    # HARD STOP: only proceed if approval is strictly ON
    approval_on = await _is_approval_on(message.chat.id)
    if approval_on is not True:
        return
...
async def toggle_flood(client, message: Message):
    ...
    if enabled:
        ...
        # Clear all flood cache for this chat when turned off
        keys_to_clear = [k for k in _flood_cache if k[0] == message.chat.id]
        for k in keys_to_clear:
            del _flood_cache[k]
        await message.reply_text("**» ❌ Flood Protection: OFF**")

# ── Flood checker — group=6 (runs AFTER approval at group=3) ─────────────────
@app.on_message(filters.group & ~filters.service & non_command & ~BANNED_USERS, group=6)
async def flood_checker(client, message: Message):
    ...
    # HARD STOP: flood must be strictly True AND limit must be set
    ...
    if len(_flood_cache[key]) > limit:
        ...
        try:
            ...
            # Reset cache so they aren't re-triggered every message after mute
            _flood_cache[key] = []
        except Exception:
            pass
